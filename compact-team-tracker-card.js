...
// Sicherstellen, dass ha-entity-picker geladen ist bevor der Editor rendert
const loadHaEntityPicker = async () => {
  if (!customElements.get("ha-entity-picker")) {
    // HA lädt den Picker lazy – wir triggern das Laden manuell
    const helpers = await window.loadCardHelpers?.();
    if (helpers) {
      await helpers.importMoreInfoControl?.("input_select");
    }
    // Fallback: direkt über den Lovelace-Import-Mechanismus
    if (!customElements.get("ha-entity-picker")) {
      await customElements.whenDefined("hui-entities-card").catch(() => {});
    }
  }
};
loadHaEntityPicker();
...
class CompactTeamTrackerEditor extends LitElement {
  ...
  render() {
    ...
    if (!this._pickerReady) return html`<div style="padding:16px;opacity:0.5;">Lade Editor...</div>`;
    const t = this._lang;
